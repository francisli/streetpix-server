  ...
  context('unauthenticated', () => {
    describe('GET /', () => {
      ...
      it('returns all public photos for a user', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?userId=1').set('Accept', 'application/json').expect(StatusCodes.OK);
      ...
      it('returns all public photos for a user by username', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?userId=admin1').set('Accept', 'application/json').expect(StatusCodes.OK);
        assert.deepStrictEqual(response.body?.length, 1);
      ...
      it('returns all public photos for a user for a given year', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?userId=2&year=2021').set('Accept', 'application/json').expect(StatusCodes.OK);
        assert.deepStrictEqual(response.body?.length, 1);
  ...
  context('authenticated', () => {
    ...
    describe('GET /', () => {
      ...
      it('returns all photos sorted by meeting date', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?sort=meeting').set('Accept', 'application/json').expect(StatusCodes.OK);
        assert.deepStrictEqual(response.body?.length, 4);
      ...
      it('returns photos with takenAt, sorted by takenAt', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?sort=takenAt').set('Accept', 'application/json').expect(StatusCodes.OK);
      ...
      it('returns photos with rating, sorted by rating', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?sort=rating').set('Accept', 'application/json').expect(StatusCodes.OK);
        assert.deepStrictEqual(response.body?.length, 2);
      ...
      it('returns photos user has rated, sorted by rating', async () => {
        /// request photos
        const response = await testSession.get('/api/photos?sort=myRating').set('Accept', 'application/json').expect(StatusCodes.OK);
        assert.deepStrictEqual(response.body?.length, 1);
